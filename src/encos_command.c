...
static int send_set_zero(const uint8_t channel, const uint8_t id) {
    log_debug("Channel %hu: send set_zero command id = %02hu", channel, id);
...
{
    log_debug("Send_motor_query: channel = %hu id = %2hu code = %2hu", channel, id, code);
    return write_can_message(channel, id, 2, (uint64_t)0x07ull << 61ull | (uint64_t)code << 48ull);
}

static int send_enable_kt(const uint8_t channel, const uint8_t id, const uint8_t enable)
{
    log_debug("Send_motor_enable_kt: channel = %hu id = %2hu code = %2hu", channel, id, enable);
    return write_can_message(channel, id, 2, (uint64_t)0xc00full << 48ull | (uint64_t)enable << 40ull);
}
...
static int parse_motor_query() /* Response type 5 */
{
    ...
    uint16_t data2      = (uint16_t)((recv_buf >> 16ull) & 0xffffull);
    log_debug("Recv response 0x05: recv_len = %hu err_no = %hu " \
            "query_code = %hu data = %hu %hu", \
...
static int parse_motor_set_range()
{
    if (recv_len != 7 || recv_id >= MOTOR_COUNT ||
        ...
        return NOT_RELATED_MSG; /* not related */
    const uint16_t minn = (uint16_t)((recv_buf >> 24ull) & 0xffffull); 
    const uint16_t maxn = (uint16_t)((recv_buf >>  8ull) & 0xffffull); 
    log_debug("Config %10s success on motor %02hu: minn = %5hu maxn = %5hu",
            config_code_to_name[(recv_buf >> 40ull) & 0xffull], recv_id,
            minn, maxn);
}

static int parse_motor_enable_kt()
{
    if (recv_len != 4 || recv_id >= MOTOR_COUNT ||
            (recv_buf >> 40ull) != 0xfffe0full)
        return NOT_RELATED_MSG; /* not related */
    log_debug("Config enable_kt success on motor %hu: enable_kt = %hu", 
            recv_id, (recv_buf >> 32ull) & 0xffull);
}
...
int set_motors_pos(const float qpos[]) {
...
int get_motors_pos_vel(float qpos[], float qvel[]) {
    const float pos_scale = 2.0f / 65535.0f;
    const float vel_scale =  2.0f / 4095.0f;
    const float offset    = 1.0f;

#pragma omp simd
    for (uint8_t j = 0; j < MOTOR_COUNT; ++j) {
        qpos[j] = (float)(current_pos_raw[j]) * pos_scale - offset;
        qvel[j] = (float)(current_vel_raw[j]) * vel_scale - offset;
    }
...
static int send_motors_set_range(const float cfg_range[2][MOTOR_COUNT],
    ...
    if (ok_cnt != MOTOR_COUNT) {
        log_warn("Set %10s: only %2hu motors send successfully",
...
int send_motors_set_pos_range(const float qpos_range[2][MOTOR_COUNT]) {
    return send_motors_set_range(qpos_range, pos_range, 100.0f, CONFIG_POS_RANGE);
}
int send_motors_set_vel_range(const float qvel_range[2][MOTOR_COUNT]) {
    return send_motors_set_range(qvel_range, vel_range, 100.0f, CONFIG_VEL_RANGE);
}
int send_motors_set_tor_range(const float qtor_range[2][MOTOR_COUNT]) {
    return send_motors_set_range(qtor_range, tor_range, 10.0f, CONFIG_TOR_RANGE);
}
int send_motors_set_cur_range(const float qcur_range[2][MOTOR_COUNT]) {
    return send_motors_set_range(qcur_range, cur_range, 10.0f, CONFIG_TOR_RANGE);
}
int send_motors_set_kp_range(const float qkp_range[2][MOTOR_COUNT]) {
    return send_motors_set_range(qkp_range, kp_range, 1.0f, CONFIG_KP_RANGE);
}
int send_motors_set_kd_range(const float qkd_range[2][MOTOR_COUNT]) {
    return send_motors_set_range(qkd_range, kd_range, 1.0f, CONFIG_KD_RANGE);
}
